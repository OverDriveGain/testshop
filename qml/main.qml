...
import QtQuick 2.12
//2.7 //(QtQuick 2.7 is lowest version for Qt 5.7)
import QtQuick.Controls 2.12
//2.0 // (requires at least Qt 5.12 where QtQuick.Controls 1 is deprecated. See https://doc.qt.io/qt-5/qtquickcontrols-index.html#versions) // needed for built-in styles // TextField, TextArea (multi-lined TextField), TextFieldStyle//import QtQuick.Controls.Material 2.12 // Other styles:
import QtQuick.Layouts 1.12
//1.15 // The module is new in Qt 5.1 and requires Qt Quick 2.1. // RowLayout, ColumnLayout, GridLayout, StackLayout, Layout
import QtGraphicalEffects 1.12
// LinearGradient
import Qt.labs.platform 1.1

// FileDialog (since Qt 5.8) // change to "import QtQuick.Dialogs" if using Qt 6.2
...
ApplicationWindow {
	...
	height: Script.getNumber(
				"neroshop.generalsettings.application.window.height")
	minimumWidth: 850
	minimumHeight: 500
	...
	header: Rectangle {
		...
		NeroshopComponents.SearchBar {
			id: searchBar
			visible: (!pageLoader.source.toString().match(
						  "qml/pages/MainPage.qml")) ? true : false
	...
	footer: Rectangle {
			...
			Rectangle {
				...
				NeroshopComponents.ProgressBar {
					...
					NeroshopComponents.Hint {
						...
						bottomMargin: footer.height + 5
						text: qsTr("neromon\n%1 (%2)").arg(
								  (parent.value < 1.0) ? "Synchronizing" : "Connected").arg(
								  (parent.value * 100).toString() + "%")
						pointer.visible: false
					}
				}
			}
			Rectangle {
				...
				NeroshopComponents.ProgressBar {
					...
					Timer {
						...
						onTriggered: {
							moneroDaemonSyncBar.value = Wallet.getSyncPercentage(
										) //Math.floor(Math.random() * (moneroDaemonSyncBar.to - moneroDaemonSyncBar.from + 1) + moneroDaemonSyncBar.from);
						}
					}
					...
					NeroshopComponents.Hint {
						...
						width: parent.width
						bottomMargin: footer.height + 5
						text: qsTr("%1\n%2 (%3)\n Blocks remaining: %4 / %5").arg("monerod").arg(
								  !Wallet.isGenerated() ? "Disconnected" : ((parent.value < 1.0) ? Wallet.getSyncMessage() : "Connected")).arg(
								  (parent.value * 100).toFixed(2) + "%").arg(
								  !Wallet.isGenerated() ? 0 : Wallet.getSyncHeight(
															  )).arg(!Wallet.isGenerated(
																		 ) ? 0 : Wallet.getSyncEndHeight()) // If connected to a remote node, "monerod" will be replaced by the <ip>:<port> of the remote node
						pointer.visible: false
					}
				}
			}
		...
		Rectangle {
			...
			Text {
				...
				property string scriptCurrency: Script.getString(
													"neroshop.generalsettings.currency")
				property string currency: Backend.isSupportedCurrency(
											  scriptCurrency) ? scriptCurrency : "usd"
				property double price: Backend.convertXmr(amount,
														  currency, false)
				text: qsTr(/*amount.toString() + " " + */ FontAwesome.monero + "  %1%2").arg(
						  Backend.getCurrencySymbol(currency)).arg(
						  price.toFixed(2)) ////.arg(currency.toUpperCase())
			...
			Text {
				...
				text: qsTr("%1%2%").arg((percent >= 0) ? "+" : "").arg(
						  percent.toFixed(2))
...
} // error: module "QtQuick.Layouts" version 1.15 is not installed (fix by changing the version specified in /usr/lib/x86_64-linux-gnu/qt5/qml/QtQuick/Layouts/plugins.qmltypes)// error: module "Qt.labs.platform" is not installed (fix: sudo apt install qml-module-qt-labs-platform)
